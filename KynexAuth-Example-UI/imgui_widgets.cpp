...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? ImColor(3, 252, 244) : ImColor(3, 252, 244), g.IO.DeltaTime * 8.f);

    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? ImColor(3, 252, 227) : ImColor(25, 25, 25, 255), g.IO.DeltaTime * 8.f);
...
//struct tab_state
//{
//    ImVec4 circle,bg,rect, shadow, line, icon, text;
//};
//
//bool ImGui::Tabs(const char* label, ImTextureID image, bool selectable, const ImVec2& size_arg)
//{
//    ImGuiWindow* window = GetCurrentWindow();
//
...
//    }
//
//    it_anim->second.icon = ImLerp(it_anim->second.icon, selectable ? c::logo_active : c::logo_in_active, g.IO.DeltaTime * 10.f);
//
//    it_anim->second.circle = ImLerp(it_anim->second.circle, selectable ? c::logo_active : c::logo_in_active, g.IO.DeltaTime * 10.f);
//
//    it_anim->second.rect = ImLerp(it_anim->second.rect, selectable ? ImColor(30,30,30,255) : ImColor(30, 30, 30, 0), g.IO.DeltaTime * 10.f);
//  
//
//  //  it_anim->second.text = ImLerp(it_anim->second.text, selectable ? c::black : c::black_in, g.IO.DeltaTime * 8.f);
//
//    GetWindowDrawList()->AddRect(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(it_anim->second.rect), 5.f);
//  
//    GetWindowDrawList()->AddShadowCircle(ImVec2(bb.Min.x + 88, bb.Min.y + 16), 8.f, GetColorU32(it_anim->second.circle), 25.f, ImVec2(0, 0));
//
//    GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Min.x + 88, bb.Min.y + 16), 4, GetColorU32(it_anim->second.circle));
//
//    //GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(38, 16), bb.Min + ImVec2(66, 44), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));
//
...
bool ImGui::Button_Icon(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    GetWindowDrawList()->AddImage(image, bb.Min, bb.Max, ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::black));

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}

extern ImFont* Montserrat_2;

struct tab_state
{
    ImVec4 multi_0, multi_1, text,line;

    float anim = 0.f;
};

bool ImGui::Tab(const char* label, bool selectable, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    ImFont* font = Montserrat_2 ? Montserrat_2 : g.Font;
    const ImVec2 label_size = font->CalcTextSizeA(font->FontSize, FLT_MAX, 0.0f, label);

    const float pad_x = 10.0f;
    const float tab_width = (size_arg.x > 0.0f) ? size_arg.x : (label_size.x + pad_x * 2.0f);
    const float tab_height = (size_arg.y > 0.0f) ? size_arg.y : 25.0f;
    const ImVec2 tab_size = ImVec2(tab_width, tab_height);

    ImVec2 pos = window->DC.CursorPos;
    const ImRect bb(pos, pos + tab_size);

    ItemSize(tab_size, 0.f);
    if (!ItemAdd(bb, id)) return false;
    ...
    ImVec4 target_text = selectable ? c::black : (hovered ? ImVec4(c::black.x, c::black.y, c::black.z, 0.95f) : c::black_in);
    ImVec4 target_bg = selectable ? c::main_color : (hovered ? ImVec4(c::main_color.x, c::main_color.y, c::main_color.z, 0.25f) : c::main_color_no_alpha);

    it_anim->second.text = ImLerp(it_anim->second.text, target_text, g.IO.DeltaTime * 12.f);
    it_anim->second.line = ImLerp(it_anim->second.line, target_bg, g.IO.DeltaTime * 12.f);

    if (it_anim->second.line.w > 0.01f)
    {
        GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.line), 5.f);
    }

    PushFont(font);
    ImVec2 text_pos = ImVec2(
        bb.Min.x + ImFloor((tab_width - label_size.x) * 0.5f),
        bb.Min.y + ImFloor((tab_height - label_size.y) * 0.5f)
    );
...
struct checkbox_state {
    ImVec4 background, background1, text, circle, circle_outline;
    float move;
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float square_sz = 24.f, w = (GetContentRegionMax().x - 13 - g.Style.WindowPadding.x) - square_sz;

    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos + ImVec2(w - 20,0), pos + ImVec2((w + square_sz), label_size.y));
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::checkbox_bg : c::checkbox_bg, g.IO.DeltaTime * 10.f);
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? color_edit4 : c::circle_checkbox, g.IO.DeltaTime * 10.f);

    it_anim->second.text = ImLerp(it_anim->second.text, *v ? c::black : hovered ? c::black : c::black_in, g.IO.DeltaTime * 10.f);
    it_anim->second.move = ImLerp(it_anim->second.move, *v ? 16.f : 0.f, g.IO.DeltaTime * 8.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(w - 8, 0), check_bb.Max + ImVec2(w, -8), GetColorU32(it_anim->second.background), 100.f);

    GetWindowDrawList()->AddCircleFilled(ImVec2(check_bb.Min.x + (w + it_anim->second.move), check_bb.Max.y - 16), 4.f, GetColorU32(it_anim->second.circle), 100.f);

    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x - square_sz + 13, check_bb.Min.y), GetColorU32(it_anim->second.text), label);

    return pressed;
}
bool ImGui::Checkbox_Esp(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float square_sz = 24.f, w = (GetContentRegionMax().x - g.Style.WindowPadding.x - 23) - square_sz;

    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2((30, 30), label_size.y + 5));
    ItemSize(total_bb, 0.f);
   ...
   /* it_anim->second.background = ImLerp(it_anim->second.background, *v ? color_edit4 : ImColor(25, 25, 25,255), g.IO.DeltaTime * 12.f);

    it_anim->second.background1 = ImLerp(it_anim->second.background1, *v ? c::black : ImColor(65, 65, 65, 255), g.IO.DeltaTime * 12.f);*/
    ...
    GetWindowDrawList()->AddCircleFilled(ImVec2(check_bb.Min.x + 5, check_bb.Max.y - 18), 8.f, GetColorU32(it_anim->second.background), 100.f);

    GetWindowDrawList()->AddCircleFilled(ImVec2(check_bb.Min.x + 5, check_bb.Max.y - 18), 4.f, GetColorU32(it_anim->second.background1), 100.f);


    return pressed;
}
//bool ImGui::Sub_Tabs(const char* label, bool selectable, const ImVec2& size_arg)
//{
//    ImGuiWindow* window = GetCurrentWindow();
//
//    if (window->SkipItems) return false;
...
//    }
//
//    it_anim->second.icon = ImLerp(it_anim->second.icon, selectable ? c::black : c::black_in, g.IO.DeltaTime * 8.f);
//
//    it_anim->second.circle = ImLerp(it_anim->second.circle, selectable ? c::circle_active : c::circle_inactive, g.IO.DeltaTime * 8.f);
//
//    it_anim->second.text = ImLerp(it_anim->second.text, selectable ? c::black : c::black_in, g.IO.DeltaTime * 8.f);
//
//    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(c::child_rect), 20.f);
//
//    GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Min.x + size_arg.x - 16, bb.Min.y + 16), 8, GetColorU32(c::circle_rect));
//
//    GetWindowDrawList()->AddShadowCircle(ImVec2(bb.Min.x + size_arg.x - 16, bb.Min.y + 16), 8.f, GetColorU32(it_anim->second.circle), 25.f, ImVec2(0, 0));
//
//    GetWindowDrawList()->AddCircleFilled(ImVec2(bb.Min.x + size_arg.x - 16, bb.Min.y + 16), 4, GetColorU32(it_anim->second.circle));
//
//    PushFont(Montserrat_2);
//
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        {
            // Draw
            window->DrawList->AddLine(bb.Min + ImVec2(13,0), ImVec2(bb.Max.x - 13, bb.Min.y), GetColorU32(ImGuiCol_Separator));
            if (g.LogEnabled)
...
struct combo_state
{
    ImVec4 background, background1, text, text_s, icon;
    ImVec4 WindowBg;
    ImVec4 Rectbg;
...
bool ImGui::BeginCombo(const char* label, ImTextureID image, const char* preview_value, float val, float width, bool multi, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(200, label_size.y));
    const ImRect bb(window->DC.CursorPos + ImVec2(230, -4), window->DC.CursorPos + ImVec2(w + 157, 18));
    ...
    //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 0), ImVec2(frame_bb.Min.x + 759, frame_bb.Min.y + 44), ImColor(57, 57, 57, 60), 6);
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (33 * val) - 4 : 0.f, g.IO.DeltaTime * 10.f);
    it_anim->second.arrow_roll = ImLerp(it_anim->second.arrow_roll, it_anim->second.opened_combo ? -0.f : 2.f, g.IO.DeltaTime * 10.f);
    it_anim->second.background = ImLerp(it_anim->second.background, it_anim->second.opened_combo || hovered ? ImColor(44, 45, 46, 255) : ImColor(44, 45, 46, 255), g.IO.DeltaTime * 10.f);
  
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? c::black : hovered ? c::black : c::black_in, g.IO.DeltaTime * 10.f);
    it_anim->second.text_s = ImLerp(it_anim->second.text_s, it_anim->second.opened_combo ? c::black : c::black, g.IO.DeltaTime * 10.f);
    it_anim->second.icon = ImLerp(it_anim->second.icon, it_anim->second.opened_combo ? color_edit4 : color_edit4, g.IO.DeltaTime * 10.f);
   ...
   // GetWindowDrawList()->AddRect(ImVec2(bb.Min.x, bb.Min.y), ImVec2(value_x2 + 20, bb.Max.y), GetColorU32(it_anim->second.background), 4.f);

    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x + 1, bb.Min.y + 1), ImVec2(value_x2 + 19, bb.Max.y - 1), GetColorU32(c::checkbox_bg), 4.f);

    GetWindowDrawList()->AddImage(image, bb.Min + ImVec2(93, 5), bb.Min + ImVec2(100, 17), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_s));

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + 3), GetColorU32(it_anim->second.text), preview_value);
 
    PopStyleColor();

    ImGui::PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x - 218, bb.Min.y + 4), GetColorU32(it_anim->second.text), label);
    ...
    PushStyleColor(ImGuiCol_WindowBg, GetColorU32(it_anim->second.WindowBg));
...
bool ImGui::Combo(const char* label, ImTextureID image, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items, float width)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!BeginCombo(label, image, preview_value, items_count, width, ImGuiComboFlags_None))
        return false;
...
bool ImGui::Combo(const char* label, ImTextureID image, int* current_item, const char* const items[], int items_count, int height_in_items, float width)
{
    const bool value_changed = Combo(label, image, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items, width);
    return value_changed;
}

// Combo box helper allowing to pass all items in a single string literal holding multiple zero-terminated items "item1\0item2\0"
bool ImGui::Combo(const char* label, ImTextureID image, int* current_item, const char* items_separated_by_zeros, int height_in_items)
{
    int items_count = 0;
    ...
    bool value_changed = Combo(label, image, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items);
    return value_changed;
}
...
struct slider_state
{
    ImVec4 text, text_number, background_search;
    float slow_anim;
};
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(w / 2 - 45, 2), window->DC.CursorPos + ImVec2(w - 45, 8));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x, 4));
    ...
    it_anim->second.text_number = ImLerp(it_anim->second.text_number, IsItemActive() ? c::black : c::black, g.IO.DeltaTime * 6.f);

    // it_anim->second.background_search = ImLerp(it_anim->second.background_search, IsItemActive() ? c::accent_color : c::slider::background_inactive, g.IO.DeltaTime * 6.f);

    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::black : hovered ? c::black : c::black_in, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 2), ImVec2(frame_bb.Max.x - 6, frame_bb.Max.y + 2), GetColorU32(c::checkbox_bg), 100.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 2), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 8, grab_bb.Max.y + 4), GetColorU32(color_edit4), 100.f);
    
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.slow_anim - 5, 0), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 12, frame_bb.Min.y + 10), GetColorU32(c::text_active), 2.f);

        //LINES
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.slow_anim - 1, 2), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, frame_bb.Min.y + 8), GetColorU32(c::input_bg), 100.f);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.slow_anim + 1, 2), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 2, frame_bb.Min.y + 8), GetColorU32(c::input_bg), 100.f);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.slow_anim + 3, 1), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 4, frame_bb.Min.y + 9), GetColorU32(c::input_bg), 100.f);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.slow_anim + 5, 2), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 6, frame_bb.Min.y + 8), GetColorU32(c::input_bg), 100.f);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.slow_anim + 7, 2), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 8, frame_bb.Min.y + 8), GetColorU32(c::input_bg), 100.f);
    }
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    PushFont(Montserrat_2);
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_number));
    RenderTextClipped(frame_bb.Min + ImVec2(31, -4), frame_bb.Max + ImVec2(31, 4), value_buf, value_buf_end, NULL, ImVec2(1.0f, 1.0f));
    PopStyleColor();
    PopFont();


    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w + 57, frame_bb.Min.y - 3), GetColorU32(it_anim->second.text), label);
...
bool ImGui::InputTextEx(const char* label, ImTextureID image, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        // Edit in progress
        const ImVec2 base_draw_pos = is_multiline ? draw_window->DC.CursorPos : ImVec2(frame_bb.Min.x + 12.0f, frame_bb.Min.y + ImFloor((frame_bb.GetHeight() - g.FontSize) * 0.5f));
        const float mouse_x = (io.MousePos.x - base_draw_pos.x) + state->ScrollX;
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? c::text_active : c::text_active, g.IO.DeltaTime * 10.f);
   ...
   // it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::checkbox_bg : c::checkbox_bg_in, g.IO.DeltaTime * 10.f);
    ...
    {
        RenderNavHighlight(frame_bb, id);
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::input_bg), 6.f);
    }
    const ImVec4 clip_rect = is_multiline ? ImVec4(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y) : ImVec4(frame_bb.Min.x + 6.0f, frame_bb.Min.y, frame_bb.Max.x - 6.0f, frame_bb.Max.y);
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : ImVec2(frame_bb.Min.x + 12.0f, frame_bb.Min.y + ImFloor((frame_bb.GetHeight() - g.FontSize) * 0.5f));
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    ImRect rect(ImVec2(rect_pos.x, draw_pos.y + bg_offy_up), ImVec2(rect_pos.x + rect_size.x, draw_pos.y + g.FontSize + bg_offy_dn));
                    rect.ClipWith(ImRect(clip_rect.x, clip_rect.y, clip_rect.z, clip_rect.w));
                    if (rect.Overlaps(ImRect(clip_rect.x, clip_rect.y, clip_rect.z, clip_rect.w)))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(0, 119, 255, 120));
                }
        ...
        {
            ImU32 col = is_displaying_hint ? GetColorU32(ImVec4(0.55f, 0.55f, 0.55f, 0.85f)) : GetColorU32(c::text_active);
        ...
        if (render_cursor)
        {
            ...
            ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y, cursor_screen_pos.x + 1.5f, cursor_screen_pos.y + g.FontSize);
            if (cursor_is_visible && cursor_screen_rect.Overlaps(ImRect(clip_rect.x, clip_rect.y, clip_rect.z, clip_rect.w))) {
                float target_offset = cursor_screen_pos.x - frame_bb.Min.x;
                if (it_anim->second.value == 0.0f) {
                    it_anim->second.value = target_offset;
                } else {
                    it_anim->second.value = ImLerp(it_anim->second.value, target_offset, ImMin(g.IO.DeltaTime * 25.0f, 1.0f));
                }
                float cursor_draw_x = frame_bb.Min.x + it_anim->second.value;
                float cursor_y1 = draw_pos.y;
                float cursor_y2 = draw_pos.y + g.FontSize;
                draw_window->DrawList->AddRectFilled(ImVec2(cursor_draw_x, cursor_y1), ImVec2(cursor_draw_x + 1.5f, cursor_y2), ImColor(255, 255, 255, 240));
            }
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    if (BeginChild("undopoints", NULL, ImVec2(0.0f, GetTextLineHeight() * 15), true)) // Visualize undo state
    {
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleVar(ImGuiStyleVar_PopupRounding, 8);
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        ImGui::GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(-3, 4), GetColorU32(c::black), label);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Render SV Square
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, 8, ImDrawCornerFlags_All);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, 8, ImDrawCornerFlags_All);
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0,8, ImDrawCornerFlags_All);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_size = GetFrameHeight() - 16;
    const ImVec2 size(size_arg.x == 0.0f ? default_size : size_arg.x, size_arg.y == 0.0f ? default_size : size_arg.y);
    float w = (GetContentRegionMax().x - g.Style.WindowPadding.x);
    const ImRect bb(window->DC.CursorPos + ImVec2(0 + w - 15, 3), window->DC.CursorPos + ImVec2(0 + w, 20) + size);
    ItemSize({ bb.Min, bb.Max - ImVec2(0, 10) }, (size.y >= default_size) ? g.Style.FramePadding.y : 0.0f);
    if (!ItemAdd(bb, id))
        return false;
    ...
    {
        off = -4.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This offset seemed like a good middle ground to reduce those artifacts.
        bb_inner.Expand(off);
    }
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddCircleFilled(bb_inner.Min - ImVec2(10, 0), 15.f, GetColorU32(col_rgb_without_alpha), 60);
    }
    else
    {
        ...
        {
            window->DrawList->AddShadowCircle(bb_inner.Min, 8.f, GetColorU32(col_source), 20.f, ImVec2(0, 0));

        

            GetForegroundDrawList()->AddCircleFilled(bb_inner.Min, 6.f, GetColorU32(col_source), 60);
          //  window->DrawList->AddCircleFilled(bb_inner.Min, 8.f, ImColor(75, 75, 90), 60);

            GetForegroundDrawList()->AddCircleFilled(bb_inner.Min, 2.f, GetColorU32(c::input_bg));
        }
        else
        {
            window->DrawList->AddShadowCircle(bb_inner.Min, 8.f, GetColorU32(col_source), 20.f, ImVec2(0, 0));

            window->DrawList->AddShadowCircle(bb_inner.Min, 4.f, GetColorU32(c::input_bg), 20.f, ImVec2(0, 0));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.Text_color = ImLerp(it_anim->second.Text_color, selected ? color_edit4 : c::text_active, g.IO.DeltaTime * 10.f);
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v",NULL, NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest,0);
    PopStyleColor(2);
